...
import AddDecision from "../decisions/add-decision";
import DecisionDetails from "../decisions/decision-details";
...
class Rule extends Component {
  ...
  editCondition(decisionIndex) {
    ...
    let outputParams = [];
    if (
      decision.event.params &&
      Object.keys(decision.event.params).length > 0
    ) {
  ...
  render() {
    ...
    const buttonProps = {
      primaryLabel: "Add Rulecase",
      secondaryLabel: "Cancel"
    };
    const editButtonProps = {
      primaryLabel: "Edit Rulecase",
      secondaryLabel: "Cancel"
    };
